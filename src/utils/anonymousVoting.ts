...
export function ApplyAnonymousProposalRulesOnProposalDetail(data: IProposalDetailData): IProposalDetailData {

    const newStatus = NewStatus(data);
    if (!!newStatus) {
        const proposalLifeList: IProposalLife[] = [{ proposalStage: AllProposalStageString.Active, proposalStatus: AllProposalStatusString.PendingCommitment }]
        return {
            ...data,
...
function NewStatus(data: { activeStartTime: string, activeEndTime: string, isAnonymous: boolean }): string | null {
    if (data.isAnonymous) {
